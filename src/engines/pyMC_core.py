...
def pytensor_transit_model(time, t0, per, rp_rs, a_ss, inc, u1, u2, ecc, cad):
    ...
    b = a_ss * pt.cos(inc * (np.pi / 180.0))
    k = rp_rs
    half_dur = (per / (2 * np.pi)) * (pt.sqrt(pt.maximum(0, (1+k)**2 - b**2)) - pt.sqrt(pt.maximum(0, (1-k)**2 - b**2)))
    is_transiting = pt.abs(phase) < half_dur
    flux_val = 1.0 - (k**2 * is_transiting)
    return flux_val
...
class BatmanOp(Op):
    ...
    def perform(self, node, inputs, outputs):
        time, t0, per, rp_rs, a_rs, inc, u1, u_2, ecc, cad = inputs
...
def median_pytensor(x):
    ...
    n = x.shape[0]
    mid = n // 2
    return pt.switch(pt.eq(n % 2, 0), (sorted_x[mid - 1] + sorted_x[mid]) / 2.0, sorted_x[mid])

def make_windows_from_time_stamps(t, gap_threshold=0.5):
    t = np.asarray(t)
    t = t[np.isfinite(t)]
    t = np.sort(t)
    if t.size == 0: return np.empty((0, 2))
    gaps = np.diff(t)
...
def sample_until_converged(model, max_attempts=3, rhat_threshold=1.1, chains=4,cores=None, mp_context="spawn"):
    ...
    for attempt in range(1, max_attempts + 1):
        ...
        run = 2+attempt
        trace = pm.sample(step=step, draws=5_000*(run), tune=2000*(run), chains=chains, cores=cores, 
...
def pymc_fit_candidate(target, candidate, time, flux, unc, verbose=False, keep_ld_if_fixed=True):
    ...
    if FORCE_CPU:
        batman_op = BatmanOp()
        model_func = lambda t, t0, per, rp_rs, a_rs, inc, u1, u2, ecc, cad: batman_op(t, t0, per, rp_rs, a_rs, inc, u1, u2, ecc, cad)
    else:
        model_func = pytensor_transit_model
    ...
    with pm.Model() as model:
        ...
        if type_fn == "Single":
            ...
            if Per_guess < 10: Per_guess = 27.8
            ...
            Per = Per_in
            if getattr(candidate, 'n_transits_obs', 0) >= 3:
                per = pm.Uniform("Per", lower=max(0.25, Per * 0.99), upper=Per * 1.01)
                a_rs = pm.Uniform("a_rs", lower=1.0, upper=300.0)
                fold_this = True
            else:
                per = pm.TruncatedNormal("Per", mu=Per, sigma=max(0.1, 0.05 * Per), lower=max(0.25, Per * 0.80), upper=Per * 1.20)
        ...
        eps = 1e-12
        rp_rs = pm.TruncatedNormal("rp_rs", mu=pt.sqrt(Depth), sigma=pt.maximum(0.02, 0.5 * pt.sqrt(Depth)), lower=0, upper=1)
        ...
        if fold_this:
            ...
            phase_arr = folded_phase[sort_indx]
            use_index = np.abs(phase_arr - T0) < min([0.5, 3*pTdur])
            dt_minutes_min = np.nanpercentile(np.diff(np.unique(np.sort(time))), 5) * 24.0 * 60.0
            p_cad = float(np.clip(dt_minutes_min, 0.2, 60.0))
            p_flux_model = model_func(phase_arr[use_index] + T0, t0, per, rp_rs, a_rs, inc, u1, u2, 0.0, p_cad)
            pm.Normal("obs", mu=p_flux_model * norm, sigma=unc[sort_indx][use_index], observed=flux[sort_indx][use_index])
            # (I will just use the original code's logic for the folding block)
            # (I will just copy the whole function from the git show)
            pass
        else:
            flux_model = model_func(time, t0, per, rp_rs, a_rs, inc, u1, u2, 0.0, cad)
